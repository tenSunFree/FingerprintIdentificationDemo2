...
public class BiometricPromptApi23 implements IBiometricPromptImpl {
    ...
    private class FingerprintManageCallbackImpl extends FingerprintManager.AuthenticationCallback {
        ...
        public void onAuthenticationError(int errorCode, CharSequence errString) {
            ...
            Log.d("more", "BiometricPromptApi23, onAuthenticationError");
            Log.d("more", "onAuthenticationError() called with: errorCode = [" + errorCode + "], errString = [" + errString + "]");
            // Vibrator myVibrator = (Vibrator) mActivity.getSystemService(Service.VIBRATOR_SERVICE);
            // myVibrator.vibrate(1000);

            // called with: errorCode = [7], errString = [嘗試次數過多，請稍後再試。]
            // called with: errorCode = [5], errString = [指紋作業已取消。]

            final int fingerprintJobCanceled = 5; // 指紋作業已取消
            final int tooManyAttempts = 7; // 嘗試次數過多, 請稍後再試

            mDialog.setState(BiometricPromptDialog.STATE_ERROR);

            switch (errorCode) {
                case fingerprintJobCanceled:
                    mManagerIdentifyCallback.onCancel();
                    break;
        ...
        public void onAuthenticationFailed() {
            ...
            Log.d("more", "BiometricPromptApi23, onAuthenticationFailed");
        ...
        public void onAuthenticationHelp(int helpCode, CharSequence helpString) {
            ...
            Log.d("more", "BiometricPromptApi23, onAuthenticationHelp");
            // mDialog.setState(BiometricPromptDialog.STATE_FAILED);
            // mManagerIdentifyCallback.onFailed();
        }
    ...
    public boolean hasEnrolledFingerprints() {
        Log.d("more", "BiometricPromptApi23, isHardwareDetected, 0");
        if (mFingerprintManager != null) {
            Log.d("more", "BiometricPromptApi23, isHardwareDetected, 1");
            return mFingerprintManager.hasEnrolledFingerprints();
        }
        Log.d("more", "BiometricPromptApi23, isHardwareDetected, 2");
        return false;
    }
}
